...
function(data,fondMaille,fondMailleElargi,fondSuppl,idData,varVolume,rayonRond,rapportRond,emprise,fondEtranger,fondChx,colPos,colNeg,colBorderPos,colBorderNeg,epaisseurBorder,opacityElargi,map_proxy){
  ...
  if(any(class(colBorderPos)!="character")) msg_error9 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(colBorderNeg)!="character")) msg_error10 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(colPos)!="character")) msg_error11 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(colNeg)!="character")) msg_error12 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(epaisseurBorder)!="numeric")) msg_error13 <- "L'epaisseur de la bordure doit etre de type numerique / "
  ...
  if(!is.null(fondMailleElargi)) if(length(names(fondMailleElargi))<3) msg_error20 <- "Le fond de maille elargie n'est pas conforme. La table doit contenir au minimum une variable identifiant, une variable libelle et la geometry / "
  if(!is.null(fondSuppl)) if(length(names(fondSuppl))<3) msg_error21 <- "Le fond supplementaire n'est pas conforme. La table doit contenir au minimum une variable identifiant, une variable libelle et la geometry / "
  ...
  if(!emprise %in% c("FRM","971","972","973","974","976","999")) msg_error25 <- "La variable emprise doit etre 'FRM', '971', '972', '973', '974', '976' ou '999' / "
  
  if (!is.null(map_proxy)) if (any(class(map_proxy)!="character")) if(!any(class(map_proxy) %in% "leaflet_proxy")) msg_error26 <- "Le parametre map_proxy doit etre un objet leaflet_proxy ou une chaine de caractere (voir aide) / "
  ...
  if(any(!is.null(msg_error1),!is.null(msg_error2),!is.null(msg_error3),!is.null(msg_error4),
         ...
         !is.null(msg_error25),!is.null(msg_error26)))
  {
    stop(simpleError(paste0(msg_error1,msg_error2,msg_error3,msg_error4,msg_error5,msg_error6,msg_error7,msg_error8,
                            ...
                            msg_error17,msg_error18,msg_error19,msg_error20,msg_error21,msg_error22,msg_error23,msg_error24,msg_error25,msg_error26)))
  }
}
