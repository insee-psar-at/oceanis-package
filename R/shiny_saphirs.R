...
function(data,fondMaille,typeMaille,fondContour,fondSuppl=NULL,idDataDepart,idDataArrivee,varFlux,direction="Ent",emprise="FRM",fondEtranger=NULL)
  {
    ...
    server <- function(input, output, session) {
      ...
      EXPORT_PROJET_QGIS_FS <- function(file)
      {
        ...
        if(rupture | min(donnees)>=0) suppressWarnings(st_write(fond_flux_entree, paste0(rep_sortie,"/fond_flux_entree.shp"), delete_dsn = TRUE, quiet = TRUE))
        if(rupture | max(donnees)<0) suppressWarnings(st_write(fond_flux_sortie, paste0(rep_sortie,"/fond_flux_sortie.shp"), delete_dsn = TRUE, quiet = TRUE))
